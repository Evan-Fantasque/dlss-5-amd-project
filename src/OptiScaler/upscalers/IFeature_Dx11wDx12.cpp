...
#include <dlssnr/amd/Performance.h>
#include <Util.h>
...
bool IFeature_Dx11wDx12::Evaluate(ID3D11DeviceContext* InDeviceContext, NVSDK_NGX_Parameter* InParameters)
{
    ...
    static const auto perf = AmdPreSr::Perf::Options::Read(Util::DllPath().parent_path());
    const double evalStart = perf.timing ? AmdPreSr::Perf::NowMs() : 0;
    double prepareMs = 0, recordMs = 0, executeMs = 0, outputMs = 0;
    AmdPreSr::Perf::inputReadyWaitMs = -1;
    AmdPreSr::Perf::bridgeSourceCall = 0;
    const auto evalTick = GetTickCount64();
    ...
    do
    {
        const double prepareStart = perf.timing ? AmdPreSr::Perf::NowMs() : 0;
        const bool prepared = ProcessDx11Textures(InParameters);
        if (perf.timing) prepareMs = AmdPreSr::Perf::NowMs() - prepareStart;
        if (!prepared)
        {
        ...
        const double recordStart = perf.timing ? AmdPreSr::Perf::NowMs() : 0;
        LOG_DEBUG("Dispatch!!");
        if (dx12Feature->GetUpscalerType() != Upscaler::DLSSD)
        {
            // ProcessDx11Textures has flushed producer copies and enqueued the
            // shared input fence wait; this freshly reset list is still empty.
            AmdPreSr::Perf::PreparedDx11Scope preparedScope;
        ...
        if (perf.timing) recordMs = AmdPreSr::Perf::NowMs() - recordStart;
    } while (false);
    ...
    if (dx12EvalResult)
    {
        ...
        const double executeStart = perf.timing ? AmdPreSr::Perf::NowMs() : 0;
        Dx12CommandQueue->ExecuteCommandLists(1, ppCommandLists);
        if (perf.timing) executeMs = AmdPreSr::Perf::NowMs() - executeStart;
    ...
    do
    {
        ...
        const double outputStart = perf.timing ? AmdPreSr::Perf::NowMs() : 0;
        const bool copied = CopyBackOutput();
        if (perf.timing) outputMs = AmdPreSr::Perf::NowMs() - outputStart;
        if (!copied)
        {
    ...
    if (perf.timing)
    {
        // Per-thread writer: no synchronization or per-frame disk opens on this path.
        static thread_local AmdPreSr::Perf::Csv csv;
        static thread_local bool opened = false;
        if (!opened)
        {
            csv.Open(Util::DllPath().parent_path() /
                (L"amd_dx11_timing_test8_" + std::to_wstring(GetCurrentProcessId()) + L"_" + std::to_wstring(GetCurrentThreadId()) + L".csv"),
                "tick_ms,source_call,frame,prepare_cpu_ms,input_ready_wait_cpu_ms,record_cpu_ms,execute_including_hip_wait_ms,output_handoff_cpu_ms,eval_before_restore_cpu_ms,success");
            opened = true;
        }
        csv.Row(evalTick, AmdPreSr::Perf::bridgeSourceCall, _frameCount, prepareMs, AmdPreSr::Perf::inputReadyWaitMs, recordMs, executeMs, outputMs,
                AmdPreSr::Perf::NowMs() - evalStart, evalResult);
    }
    if (evalResult)
        _frameCount++;
    else
