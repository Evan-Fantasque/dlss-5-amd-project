...
bool Dx11WithDx12::SyncDx11ToDx12()
{
    ...
    // Experimental AMD inline path: retire only the already-flushed D3D11
    // producer before waking HIP. Keep the GPU queue wait below in both modes.
    // No resource copies or temporal resets are added. On failure do not record NR.
    static const auto perf = AmdPreSr::Perf::Options::Read(Util::DllPath().parent_path());
    AmdPreSr::Perf::inputReadyWaitMs = -1;
    if (perf.inputReady && Config::Instance()->DlssNrEnabled.value_or_default() &&
        DlssNr::AmdBridge::HasFiles())
    {
        const double started = perf.timing ? AmdPreSr::Perf::NowMs() : 0;
        result = AmdPreSr::Perf::WaitForInput(Dx12FenceTextureCopy, fenceValue, Dx12Device);
        if (perf.timing) AmdPreSr::Perf::inputReadyWaitMs = AmdPreSr::Perf::NowMs() - started;
        if (FAILED(result))
        {
            LOG_ERROR("AMD D3D11 input fence wait failed: {:X}", (UINT) result);
            return false;
        }
    }
